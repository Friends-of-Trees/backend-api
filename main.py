...
async def assign_winner(data: WinnerAssignment):
    try:
        response = (
            supabase.table("competition_entries")
            .update({"winner_rank": data.winner_rank})
            .eq("id", data.entry_id)
            .execute()
        )
        ...
        return {"status": "success", "message": f"Awarded {data.winner_rank}"}
...
async def submit_entry(
    ...
    for photo in photos:
        ...
        if mime_type not in {"image/jpeg", "image/png"}:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported file: {photo.filename}"
            )
...
@app.get("/admin/entries")
def get_entries():
    entries = supabase.table("competition_entries") \
        .select("*") \
        .order("created_at", desc=True) \
        .execute().data

    for entry in entries:
        images = supabase.table("entry_images") \
            .select("image_url") \
            .eq("entry_id", entry["id"]) \
            .execute().data
...
def scan_ai_entries():
    ...
    for entry in entries:
        ...
        try:
            ai_status = is_ai_image_from_url(images[0]["image_url"])
...
# -------------------------------------------------
# DETECT AI (single file)
# -------------------------------------------------
@app.post("/detect-ai")
async def detect_ai(file: UploadFile = File(...)):
    try:
        contents = await file.read()
        # ⚠ adjust depending on your model
        result = is_ai_image_from_url(contents)
        return {"is_ai": result}
    except Exception as e:
        return {"error": str(e)}
